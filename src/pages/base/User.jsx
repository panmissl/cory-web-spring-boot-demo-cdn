...
const Page = () => {
  ...
  const operationList = [
    {
      label: '分配角色',
      handler: (record, actionRef) => setupAssignRole(record, true, actionRef),
      type: 'normal',
      icon: <AuditOutlined />,
    },
    {
      label: '修改密码',
      handler: (record, actionRef) => setupChangePassword(record, true, actionRef),
      type: 'normal',
      icon: <SafetyOutlined />,
    },
  ];
    ...
    <PageContainer>
      ...
      {currentUser && (
        <Modal
          ...
          footer={
            <>
              <Button type="primary" onClick={() => assignRole()}>
                保存
              </Button>
      ...
      {currentUser && (
        ...
        >
          <div>
            请输入新密码(明文密码，比如：123456)[长度在6-32位之间，前面和后面的空格会自动删除]
          </div>
          <Input
            value={password}
            onChange={(e) => setPassword(e.target.value)}
            placeholder="请输入新密码(明文密码，比如：123456)[长度在6-32位之间，前面和后面的空格会自动删除]"
          />
        </Modal>
      )}
    </PageContainer>
